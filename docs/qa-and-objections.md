...
esrs-transition-agent is a **Column S staging** agent. Customers will still ask why a row was classified, or ask for metric names, or ask general ESRS questions. Captured v2 behavior:
...
The agent answered from Column D/E (numbered AR with a Draft ID ≠ section header / objective framing). It offered to change the decision before write. That is the intended **Review** path: explain, optionally edit S, then return to `Write this batch?`.

## Metric naming (out of scope)

If they keep Add to Program but ask the agent to draft a metric name:

![Refuse to name metrics](screenshots/qa-108-metric-name-out-of-scope.png)

Correct: the agent does **not** write T/U or invent names. They name metrics in the program.
...
A **new** chat (or before start-row / first scan) can take off-workflow questions. The agent called **Revised ESRS** and answered (waves, simplification, E1, citations).
...
- In-batch → only row objections and S decision edits (Review). Do not ask the agent to switch into open Q&A mid-batch.
